...
public class SAOExtractor {
	public List<SAOTuple> getSAOTupleList(String data) throws InvalidFormatException, IOException{
	...
	public SAOTuple getSAOTuple(Tree clause) {
		...
		return new SAOTuple(getText(clause), getText(subject),getText(predicate),getText(object));
	}
	...
	private Tree getSubject(Tree tree) {
		...
		while (!queue.isEmpty()) {
			Tree p = queue.poll();
			List<Tree> list = getChildrenByType(p, "NP");
			if (getType(p,"NP") && list.isEmpty()) {
				String[] types = { "NN", "NNP", "NNPS", "NNS" };
				List<Tree> children = getChildrenByType((p), types);
				if (!children.isEmpty()) {
